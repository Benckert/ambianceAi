...
export const LayerControl = ({ layer, variant = "manual" }: LayerControlProps) => {
    ...
    <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 bg-gray-800 rounded-lg">
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            {isMuted ? (
              <VolumeX size={18} className="text-red-400 sm:w-5 sm:h-5" />
            ) : (
...
export const LayersList = ({ variant = "manual" }: { variant?: "manual" | "ai" | "semantic" }) => {
  ...
  return (
    <div className="space-y-2 sm:space-y-3">
      {layers.map((layer) => (
        <LayerControl key={layer.id} layer={layer} variant={variant} />
      ))}
    </div>
  )
}
